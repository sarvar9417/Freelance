  ...
  if (loadingProfile) {
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <Loader2 className={`h-8 w-8 animate-spin ${isDark ? 'text-white/20' : 'text-gray-300'}`} />
      </div>
    )
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex items-center gap-3 mb-8">
        <Link href="/profile" className={`inline-flex items-center gap-2 text-sm transition-colors ${isDark ? 'text-white/40 hover:text-white/70' : 'text-gray-500 hover:text-gray-900'}`}>
          <ArrowLeft className="h-4 w-4" />
          Profilga qaytish
        </Link>
        <span className={isDark ? 'text-white/15' : 'text-gray-300'}>/</span>
        <span className={`text-sm ${isDark ? 'text-white/60' : 'text-gray-600'}`}>Sozlamalar</span>
      </div>
      ...
      <div className="grid lg:grid-cols-4 gap-6">
        ...
        <nav className="lg:col-span-1 space-y-1">
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-left transition-all ${
                  isActive
                    ? isDark ? 'text-white border border-blue-500/30' : 'text-blue-700 border border-blue-200 bg-blue-50'
                    : isDark ? 'text-white/40 hover:text-white/70 border border-transparent hover:border-white/8' : 'text-gray-500 hover:text-gray-900 border border-transparent hover:border-gray-200 hover:bg-gray-50'
                } ${s.id === 'danger' ? '!text-rose-500 hover:!text-rose-600' : ''}`}
                style={isActive && isDark ? { background: 'rgba(59,130,246,0.1)' } : (!isActive && isDark ? { background: 'rgba(255,255,255,0.02)' } : {})}
              >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border border-transparent hover:border-rose-500/20 hover:text-rose-500 transition-all mt-4 ${isDark ? 'text-white/30' : 'text-gray-400'}`}
            style={isDark ? { background: 'rgba(255,255,255,0.02)' } : {}}
          >
            ...
            >
              ...
              {section === 'profile' && (
                <div className="space-y-1">
                  <h2 className={`font-semibold text-base flex items-center gap-2 mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <User className="h-4 w-4 text-blue-400" />Profil ma&apos;lumotlari
                  </h2>
                  <p className={`text-sm mb-6 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Ism, yosh va bio ni tahrirlang</p>
                  {userId && profile ? (
                    <EditProfileForm
                      userId={userId}
                      initialData={profile}
                    ...
                    <div className="flex items-center justify-center py-12">
                      <Loader2 className={`h-5 w-5 animate-spin ${isDark ? 'text-white/20' : 'text-gray-300'}`} />
                    </div>
                  )}
                </div>
              )}
              ...
              {section === 'password' && (
                <div>
                  <h2 className={`font-semibold text-base flex items-center gap-2 mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <Lock className="h-4 w-4 text-blue-400" />Parolni o&apos;zgartirish
                  </h2>
                  <p className={`text-sm mb-6 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Xavfsizlik uchun kuchli parol ishlating</p>
                  ...
                  <form onSubmit={handlePasswordChange} className="space-y-4">
                    {[
                      ...
                      <div key={f.label} className="space-y-1.5">
                        <label className={`text-xs font-semibold uppercase tracking-wider ${isDark ? 'text-white/45' : 'text-gray-500'}`}>{f.label}</label>
                        <div className="relative">
                          <input
                            ...
                            className={`w-full px-4 py-3 pr-10 rounded-xl text-sm outline-none focus:ring-1 focus:ring-blue-500/50 transition-all ${isDark ? 'text-white placeholder:text-white/20' : 'text-gray-900 placeholder:text-gray-400 bg-white border border-gray-200'}`}
                            style={isDark ? { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' } : {}}
                          />
                          <button type="button" onClick={() => setShowPw(v => !v)}
                            className={`absolute right-3 top-1/2 -translate-y-1/2 transition-colors ${isDark ? 'text-white/25 hover:text-white/50' : 'text-gray-400 hover:text-gray-600'}`}>
                    ...
                    {newPw && (
                      <div className="space-y-1">
                        <p className={`text-[10px] ${isDark ? 'text-white/30' : 'text-gray-400'}`}>Parol kuchi</p>
                        <div className="flex gap-1">
                          {['bg-rose-500', 'bg-amber-500', 'bg-emerald-500'].map((color, i) => (
                            <div key={i} className={`h-1.5 flex-1 rounded-full transition-all ${
                              newPw.length >= (i + 1) * 4 ? color : (isDark ? 'bg-white/10' : 'bg-gray-200')
                            }`} />
                          ))}
                        </div>
              ...
              {section === 'notifications' && (
                <div>
                  <h2 className={`font-semibold text-base flex items-center gap-2 mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <Bell className="h-4 w-4 text-blue-400" />Bildirishnomalar
                  </h2>
                  <p className={`text-sm mb-6 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Qaysi bildirishnomalarni olishni tanlang</p>
                  <div className="space-y-3">
                    {NOTIF_OPTIONS.map(({ key, label, desc }) => (
                      <div key={key} className={`flex items-center justify-between p-4 rounded-xl ${isDark ? '' : 'bg-gray-50 border border-gray-200'}`}
                        ...
                        <div>
                          ...
                          <p className={`text-xs mt-0.5 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>{desc}</p>
                        </div>
                        <Toggle
                          checked={notifs[key]}
              ...
              {section === 'language' && (
                <div>
                  <h2 className={`font-semibold text-base flex items-center gap-2 mb-1 ${isDark ? 'text-white' : 'text-gray-900'}`}>
                    <Globe className="h-4 w-4 text-blue-400" />Interfeys tili
                  </h2>
                  <p className={`text-sm mb-6 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Qulay tilni tanlang</p>
                  <div className="space-y-3">
                    {LANGUAGES.map(l => (
                      <button key={l.code} onClick={() => saveLang(l.code)}
                        className={`w-full flex items-center gap-4 p-4 rounded-xl text-left transition-all ${
                          lang === l.code
                            ? isDark ? 'border border-blue-500/30' : 'border border-blue-300 bg-blue-50'
                            : isDark ? 'border border-white/6 hover:border-white/12' : 'border border-gray-200 hover:border-gray-300 bg-white'
                        }`}
                        style={lang === l.code && isDark ? { background: 'rgba(59,130,246,0.1)' } : (lang !== l.code && isDark ? { background: 'rgba(255,255,255,0.03)' } : {})}>
                        <span className="text-2xl">{l.flag}</span>
                        <span className={`text-sm font-medium ${lang === l.code ? (isDark ? 'text-white' : 'text-blue-700') : (isDark ? 'text-white/60' : 'text-gray-600')}`}>
                          {l.label}
                        </span>
                        {lang === l.code && (
              ...
              {section === 'danger' && (
                <div>
                  ...
                  <p className={`text-sm mb-6 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Bu amallar qaytarib bo&apos;lmaydi</p>

                  <div className="space-y-4">
                    <div className={`p-4 rounded-xl flex items-start justify-between gap-4 ${isDark ? '' : 'bg-gray-50 border border-gray-200'}`}
                      style={isDark ? { background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' } : {}}>
                      <div>
                        <p className={`text-sm font-medium ${isDark ? 'text-white/80' : 'text-gray-800'}`}>Ma&apos;lumotlarni eksport qilish</p>
                        <p className={`text-xs mt-0.5 ${isDark ? 'text-white/35' : 'text-gray-500'}`}>Barcha ma&apos;lumotlaringizni JSON formatda yuklab oling</p>
                      </div>
                    ...
                    <div className="p-4 rounded-xl"
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div>
                          <p className="text-rose-500 text-sm font-semibold">Akkauntni o&apos;chirish</p>
                      ...
                      <div className="space-y-3">
                        <input
                          ...
                          className={`w-full px-3.5 py-2.5 rounded-xl text-sm outline-none ${isDark ? 'text-white placeholder:text-white/20' : 'text-gray-900 placeholder:text-gray-400'}`}
