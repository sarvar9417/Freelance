...
export default function TaskFormClient({ courseId, courseTitle, lessons, initial, mode = 'create' }: Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await createTask(courseId, {
        title: form.title,
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <h1 className={`text-xl font-bold ${isDark ? 'text-white' : 'text-gray-900'}`}>
            {mode === 'create' ? 'Yangi topshiriq yaratish' : 'Topshiriqni tahrirlash'}
          </h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        >
          ...
          {lessons.length > 0 && (
            <div>
              ...
              <select value={form.lesson_id} onChange={e => set('lesson_id')(e.target.value)}
                className={`w-full px-4 py-2.5 rounded-xl text-sm outline-none appearance-none focus:ring-1 focus:ring-amber-500/500 ${isDark ? 'text-white' : 'text-gray-900'}`}
                style={isDark ? { background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.08)' } : { background: 'white', border: '1px solid #e5e7eb' }}>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <button
              ...
              className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all border ${
                isDark ? '' : 'bg-blue-50 border-blue-200 text-blue-600'
              }`}
          ...
          {uploadError && (
            <p className={isDark ? 'text-red-400 text-xs mb-2' : 'text-red-600 text-xs mb-2'}>{uploadError}</p>
          )}
        ...
        <div className="flex gap-3">
          ...
          <button type="submit" disabled={isPending || uploading}
            className={`flex-1 py-3 rounded-xl text-sm font-medium transition-all disabled:opacity-50 flex items-center justify-center gap-2 ${
            ...
            {mode === 'create' ? 'Topshiriq yaratish' : "O'zgarishlarni saqlash"}
          </button>
        </div>
      </form>
    </div>
