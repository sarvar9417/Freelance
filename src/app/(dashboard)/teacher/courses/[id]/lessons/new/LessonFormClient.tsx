...
interface Props {
  ...
  courseTitle: string
  nextOrder: number
  initial?: { id: string; title: string; order_num: number; video_url: string; content: string }
  mode?: 'create' | 'edit'
}

export default function LessonFormClient({ courseId, courseTitle, nextOrder, initial, mode = 'create' }: Props) {
  const router = useRouter()
  const [isPending, startTransition] = useTransition()
  const [error, setError] = useState('')
  const [form, setForm] = useState({
    title: initial?.title ?? '',
    order_num: initial?.order_num ?? nextOrder,
